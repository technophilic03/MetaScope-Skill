...
The skill targets **Rutgers Amarel** — the standard research HPC at Rutgers, operated by OARC (Office of Advanced Research Computing), running SLURM. Site-specific values (account format, partitions, modules, scratch paths) are NOT baked into the skill — the user supplies them once via `~/.config/metascope-microbiome/rutgers.yaml` and they're reused on every invocation.
...
If the lab uses a Rutgers cluster other than Amarel, the SLURM directives in `assets/slurm_template.sh.j2` likely still apply (Amarel and most Rutgers clusters are SLURM); only the account/partition/module names differ. The other half of the skill (samplesheet builder, validators, registry, the `nextflow run nf-core/metascopeprolifer` line embedded in `submit.sh`) is scheduler- and cluster-agnostic.
...
If your cluster is **not** SLURM (PBS/Torque/etc.), the SLURM template in `assets/slurm_template.sh.j2` will not apply — file an issue and adapt. The rest of the skill (samplesheet builder, validators, registry, the `nextflow run nf-core/metascopeprolifer` invocation it embeds) is scheduler-agnostic.

## Generic vs. site-specific (the dividing line)

| Generic — baked into `assets/slurm_template.sh.j2` | Site-specific — user fills in `rutgers_config.yaml` |
|----------------------------------------------------|-----------------------------------------------------|
| `#SBATCH` directive syntax                         | `--account` value                                   |
| stdout/stderr filename pattern (`%x-%j.out`)       | `--partition` value                                 |
| Job-name pattern                                   | `--time`, `--mem`, `--cpus-per-task` defaults       |
| Exit-code propagation                              | `module load` lines                                 |
| `nextflow run <ref>` invocation pattern            | nf-core profile (`singularity,slurm`, etc.)         |
| `fastq-dump --split-files --gzip` invocation       | Pipeline ref (Howard's repo URL or local path)      |
| Samplesheet column names (from Howard's pipeline)  | Scratch / work / output directory paths             |
|                                                    | QoS (if your group uses one)                        |

## How the skill collects these values

Two paths, used together or separately:

1. **Interactive** (default) — Claude asks the user one question per field at run time, suggesting hints/options where possible (`partition`: `main`/`cmain`/`gpu`; `nextflow_profile`: `singularity,slurm`/`apptainer,slurm`; …). Answers feed `render_submission.py` directly via CLI flags (`--account`, `--partition`, `--module-loads`, …).
2. **Cache** (optional) — after the first interactive session, the skill offers to write the answers to `~/.config/metascope-microbiome/rutgers.yaml`. On future runs, pass `--rutgers-config <that path>` to seed defaults. Any individual value can still be overridden with a CLI flag for an ad-hoc run (e.g. `--partition gpu` to switch partitions for one run while keeping the rest of the cache).
...
| `default_mem`       | Memory request (e.g., `64G`).                                                          |
...
| `nextflow_profile`  | Nextflow profile string. Often `singularity,slurm`. Confirm Rutgers' convention.       |
| `pipeline_ref`      | `hjfan527/nf-core-metascopeprolifer`, a tagged commit SHA, or a local clone path.      |
...
## Verification checklist (run once after filling rutgers.yaml)
